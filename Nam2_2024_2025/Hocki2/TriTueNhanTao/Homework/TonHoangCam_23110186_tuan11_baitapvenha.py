...
def A(start_state, goal):
    ...
    visited.add(tuple(map(tuple, start_state)))
    while priority_queue:
        _, manhattan, cost, current_state, path = heapq.heappop(priority_queue)
        if current_state == goal:
            return path + [current_state]
        for next_state in move(current_state):
            state_tuple = tuple(map(tuple, next_state))
...
def steepest_climb(start_state, goal):
    ...
    while True:
        ...
        if not neighbors:
            return path
        best_neighbor = min(neighbors, key=lambda x: khoangcach_manhattan(x, goal))
        if khoangcach_manhattan(best_neighbor, goal) < khoangcach_manhattan(current_state, goal):
...
def stochastic_climb(start_state, goal):
    ...
    while True:
        if current_state == goal:
            return path
        neighbors = move(current_state)
        better_neighbors = [n for n in neighbors if khoangcach_manhattan(n, goal) < khoangcach_manhattan(current_state, goal)]
        if better_neighbors:
