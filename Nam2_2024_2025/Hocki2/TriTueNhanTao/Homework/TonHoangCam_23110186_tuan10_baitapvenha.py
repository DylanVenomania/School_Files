...
def simulated_annealing( start_state, goal):
    ...
    cooling_rate = 0.95  # Tỷ lệ giảm nhiệt độ
    ...
    for _ in range(max_iterations):
        ...
        # Nếu giải pháp tốt hơn hoặc chấp nhận giải pháp xấu với xác suất
...
def fitness(state, goal):
    return -khoangcach_manhattan(state, goal)  # Fitness cao hơn khi Manhattan thấp hơn
...
def ditruyen(start_state, goal, pop_size=50, max_generations=100, mutation_rate=0.1):
    ...
    for _ in range(max_generations):
        ...
        new_population = [fitness_scores[0][0][:]]  # Giữ cá thể tốt nhất
